/** XTRM footer: git/model context plus one compact cached beads status line.
 *
 * xtrm-64pl0: the footer is a pure cache READER. It renders path/branch, context/model,
 * and a single compact beads line produced by `formatCompact` from the on-disk cache
 * (.xtrm/cache/beads-status.json). It never spawns bd, never refreshes the cache via a
 * subprocess, and exposes no expandable tree/toggle. The cache is regenerated by beads
 * hooks elsewhere; stale/unavailable is acceptable and rendered as-is.
 */
