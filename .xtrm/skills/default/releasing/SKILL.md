...
wrapper. Assumes `[Unreleased]` in `CHANGELOG.md` is already populated — either
by the `session-close-report` skill (Step 6) across the contributing sessions,
or auto-generated by the `/setup-git-cliff` pipeline.
...
- **Populated and complete** (covers the user-facing changes since last tag):
  proceed to Step 3.
- **Empty, stale, or gappy, and this repo runs the git-cliff pipeline**
  (`changelog/cliff.toml` + `scripts/changelog-update.mjs` — see
  `/setup-git-cliff`): run `node scripts/changelog-update.mjs` to regenerate
  `[Unreleased]` from the git log, then re-inspect.
...
- If this repo runs the git-cliff pipeline (`changelog/cliff.toml` +
  `scripts/changelog-update.mjs` — see `/setup-git-cliff`), the promoted
  bullets are auto-generated per-commit and terse. Draft a short prose
