...
This skill was extracted from the `mmd-sprint 2026-07-03` `judge.txt` protocol (see `/tmp/mmd-sprint/judge.txt`) after the sprint that closed the 82wh regression. The rules below survived that sprint; do not paraphrase them into softer language.

## Prerequisites

Load these skills **first**, in this order, before running any review:

1. `/multiplexing` — you are a helper in a multiplexed session; know the reporting protocol.
2. `/code-review` — the underlying review discipline. Every PR verdict is a `/code-review` output shaped through the verdict rubric here.
3. `/multiplexing-team` — the upward-reporting protocol you use to file verdicts.

If you are running from a Codex or pi pane and one of these skills does not resolve by short name, fall back to the filesystem path (`~/.claude/skills/<name>/SKILL.md` or `~/dev/core/skills/<name>/SKILL.md`). Do not proceed without them.
...
Do **not** overwrite prior notes on the same bead — `bd update --notes` appends by default in the Mercury workflow, but confirm with `bd show <bead>` before writing that a re-review has been persisted.
...
Extracted 2026-07-03 from `/tmp/mmd-sprint/judge.txt` (mmd-sprint 2026-07-03). Follow-up bead: `mercury-market-data-71or` (`/judge-with-codex` skill extraction, P2).
