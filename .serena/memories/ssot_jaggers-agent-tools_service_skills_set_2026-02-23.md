...
| Section | Summary |
|---|---|
| [Location](#location) | The canonical source for project-specific Claude Code service skill infrastructure |
| [Architecture](#architecture) | Three workflow skills are installed into ` |
| [Hook Wiring (settings.json)](#hook-wiring-settings.json) | Three Claude Code hooks wired at install time: |
| [3-Phase Workflow (creating-service-skills)](#3-phase-workflow-creating-service-skills) | Claude fills all `[PENDING RESEARCH]` markers using Serena LSP tools: |
| [Installer](#installer) | Path constants in installer (after `service-skills-set/` reorganization): |
| [Key Constraints](#key-constraints) | All scripts that need registry access import bootstrap: |
| [Bootstrap Cross-Reference Pattern](#bootstrap-cross-reference-pattern) | All scripts that need registry access import bootstrap: |
<!-- END INDEX -->
