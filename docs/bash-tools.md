...
version: 1.1.0
updated: 2026-07-16
description: "Bash-native companion commands for GitHub code search, Context7, and DeepWiki"
source_of_truth_for:
  - "scripts/ghgrep.mjs"
...
| `ctx7` | Context7 docs/skills CLI for library lookup and skill management | Machine bootstrap (`npm install -g ctx7`) via `xt init` | `context7` MCP/plugin workflows |
| `deepwiki` | DeepWiki CLI for repo wiki/doc Q&A | Machine bootstrap (`npm install -g @seflless/deepwiki`) via `xt init` | `deepwiki` MCP server |
...
```bash
npm install -g github:xtrm-dev/core@latest
```
