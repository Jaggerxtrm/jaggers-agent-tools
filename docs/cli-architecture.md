...
updated: 2026-07-17
synced_at: a7a3caa3
description: "Current xtrm CLI command map, phased init flow, and core module responsibilities"
source_of_truth_for:
  - "cli/src/**/*.ts"
domain: [cli]
...
| [Init Flow (`xtrm init`)](#init-flow-xtrm-init) | Preflight, plan, confirmation, runtime sync, project bootstrap, and verification |
...
```
xt / xtrm
├── claude, pi       → runtime launchers and runtime management
├── init             → phased machine/runtime/project bootstrap
├── status, doctor   → health and drift reporting
├── reset, clean     → configuration reset and orphan cleanup
├── end, merge       → worktree publication and PR queue management
├── worktree, attach → worktree inventory, audits, cleanup, and re-attachment
├── docs             → show, list, cross-check, verify
├── memory           → project-memory synthesis
├── debug            → hook and bead event monitoring
├── report           → generate, show, list, diff
├── skills           → list, enable, disable, create-pack
├── claude-sync      → managed CLAUDE.md fragment reconciliation
├── bootstrap        → populate global xtrm payloads
├── update           → refresh managed assets and Pi packages
├── release          → prepare and publish release flow
├── spec             → draft, validate, doctor, apply, status, archive
├── migrate          → one-time global-scope migration
└── help             → plain-text command reference
```
...
`runProjectInit()` in `commands/init.ts` is the supported bootstrap entry point:

```
runProjectInit()
  ├── preflight          → inventory machine, runtime, skills, bd, and GitNexus state
  ├── plan + confirm     → render one consolidated plan and gate all mutations
  ├── machine bootstrap → install missing workflow prerequisites
  ├── Claude sync       → reconcile managed hooks and settings
  ├── registry scaffold → install package-owned .xtrm assets
  ├── Pi sync           → MCP config, extensions, packages, and runtime hooks
  ├── skills views      → rebuild global/project active views and runtime pointers
  ├── project bootstrap → bd, GitNexus, agent instructions, service-skill wiring
  ├── maintenance       → refresh stale bd/GitNexus state
  └── verification      → emit one cross-phase health summary
```

`xt update --apply` is the refresh path for an already-initialized repo. `xt bootstrap` only populates the user-global payload from the installed package.
...
The Pi runtime module owns the canonical xt-managed package inventory and package health contracts. Use `getXtManagedPiPackages()` rather than hard-coded package allowlists. The inventory currently includes `npm:@jaggerxtrm/pi-extensions`, `npm:pi-gitnexus`, `npm:@zenobius/pi-worktrees`, `npm:@robhowley/pi-structured-return`, `npm:@aliou/pi-guardrails`, `npm:@aliou/pi-processes`, and `npm:pi-mcp-adapter`.
...
- **`xt skills list [--global|--local] [--claude|--pi] [--json]`**: shows tiered skill inventory. Reads from `.xtrm/skills/default/` plus optional/user packs (optional pack directories are pre-populated by `xt init`, then runtime-activated via `xt skills enable <pack>`). Emits `defaultSkills`, `packs[]`, and `runtimeStatus[]` per runtime.
...
1. **PROTECTED_KEYS** (`hooks.PreToolUse`, `hooks.SessionStart`,
   ...
   This prevents `xtrm init` from overwriting user hook customisations.
...
overwrite. Known limitation: this means `xtrm init` is additive-only for
